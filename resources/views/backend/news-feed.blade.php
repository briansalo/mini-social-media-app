...
<!-- end Delete Modal -->
	 ...
	 @foreach($post as $key => $data)
				...
				<div class="news-feed-header justify-content-between d-flex ps-3 pt-2">
					...
					<div class="d-flex">
						...
						@if(!empty($data->user_info->profile_photo))	
							<img src="{{url('/upload/profile/'.$data->user_info->profile_photo)}}" class="rounded-circle" 
			...
			<!--------------------------Tagged Features------------------------------>
					...
					@else
						<!--if the post has tag by this current user-->
							@foreach($data->users as $tag)
								 @if(Auth()->user()->id == $tag->id)
									<div>
										  <a class="btn btn-light " data-bs-toggle="dropdown" aria-expanded="false">
						    				<i class="fas fa-ellipsis-h" ></i>
										  </a>
										  <div class="dropdown-menu dropdown-menu-end">							     
							  		 <a class="dropdown-item" href="{{ route('remove.tag', $data->id)}}" >Remove Tag</a>
										  </div>
									</div>	
								 @endif
							@endforeach		
